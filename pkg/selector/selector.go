...
// letters: `-v -v` and `-vvv` both hold -vv. An option written with a
// value, `--format=long` or `-func=c.out`, holds its name and "=" as well. A "--" ends the options:
...
func matchArgs(a *definition.ArgsMatch, args []string) (reason, excluded string, ok bool) {
	...
	for _, arg := range args {
		...
		set[arg] = true
		// An option with a value also holds the option with its "=", so
		// `none: ["--config-env="]` refuses it whatever the value. Go's
		// flag package writes a long option with one dash, so
		// `-func=c.out` holds "-func=" the same way.
		if strings.HasPrefix(arg, "-") {
			if i := strings.IndexByte(arg, '='); i > 1 && arg[:i] != "--" {
				set[arg[:i+1]] = true
			}
		}
