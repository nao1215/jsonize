...
func TestLoadErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{"tree indented with something other than whitespace", "format: 1\ncommand: c\nvariant: v\nparse: {type: tree, indent: \"|-\", node: {parse: {type: regex, pattern: '(?P<a>.)'}}}\n", "must be spaces or tabs"},
		...
		{"tree inside records", "format: 1\ncommand: c\nvariant: v\nparse: {type: records, start: '^x', parts: [{name: p, parse: {type: tree, indent: \"  \", node: {parse: {type: regex, pattern: '(?P<a>.)'}}}}]}\n", "records part cannot be a tree"},
