...
- `openssl x509 -noout -text` (`openssl/x509-text`): the version, the serial, the signature algorithm, the issuer, the subject, the two dates, the kind of key and its size, and one record per X509v3 extension with its critical mark and its value as printed. The key material and the signature are hexadecimal folded over a dozen lines or more and are not read; `input.ignore` names those lines, so `--explain` reports how many were left out. The registry holds 270 commands through 745 definitions.
