...
- YAML: the value of an explicit key written on the line of its ":" was read as one string when it opened a collection there. `? a` followed by `: - x` and `  - y` gave `{"a":"- x - y"}` with exit 0; it is now the list `["x","y"]`, and `: b: 1` is a mapping.
