...
func TestComplete(t *testing.T) {
	...
	tests := []struct {
		...
		{[]string{"-"}, completeWords, []string{"--parser", "--variant", "--file", "-f", "--format", "--stream", "--columns", "--explain", "--explain=json", "--help", "--version"}, []string{"--env", "--timeout", "--yaml"}},
		...
		{[]string{"new", "-"}, completeWords, []string{"--array", "--pretty"}, []string{"--file", "--stream", "--yaml"}},
		...
		{[]string{"--raw", "--pa"}, completeWords, []string{"--parser"}, nil},
		{[]string{"run", ""}, completeWords, []string{"df", "ls", "ping", "ping6", "curl"}, []string{"run"}},
		{[]string{"run", "-"}, completeWords, []string{"--env", "--timeout", "--keep-locale"}, []string{"--yaml"}},
