...
func TestParseValues(t *testing.T) {
	...
	tests := []struct {
		...
		{"explicit key with a sequence after its colon", "? a\n: - x\n  - y\nb: 1\n", map[string]any{"a": []any{"x", "y"}, "b": int64(1)}},
		{"explicit key with a mapping after its colon", "? a\n: b: 1\n  c: 2\n", map[string]any{"a": map[string]any{"b": int64(1), "c": int64(2)}}},
		...
		{"literal ending the text without a line break", "a: |\n  x\n  y", map[string]any{"a": "x\ny"}},
		{"folded ending the text without a line break", "a: >\n  x\n  y", map[string]any{"a": "x y"}},
		{"kept literal ending the text without a line break", "a: |+\n  x", map[string]any{"a": "x"}},
		{"kept literal with empty lines at the end", "a: |+\n  x\n\n", map[string]any{"a": "x\n\n"}},
		...
		{"trailing spaces", "a: 1   \nb: x  # c\n", map[string]any{"a": int64(1), "b": "x"}},
		{"dash inside a value", "a: b - c\n", map[string]any{"a": "b - c"}},
		{"dash glued to a value", "a: -b\nc: [-d]\n", map[string]any{"a": "-b", "c": []any{"-d"}}},
...
func TestParseErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{"sequence item on the line of its key", "a: - b\n", 1, "sequence item"},
		{"lone dash on the line of its key", "a: -\n", 1, "sequence item"},
		{"explicit key where a value is", "a: ? b\n", 1, "explicit key"},
		{"sequence item in a flow sequence", "a: [-1, - 1]\n", 1, "sequence item"},
		{"line after a comment continuing a plain scalar", "a: x # c\n  y\n", 2, "unexpected indentation"},
		{"line after a comment continuing a sequence item", "- x # c\n  y\n", 2, "unexpected indentation"},
