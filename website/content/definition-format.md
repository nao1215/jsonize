...
- `aligned`: cells are cut where the header words start. A value that
  ...
  ("Mounted on") belong to the last column, and a heading of several
  words one space apart is one column when `columns` gives it the name
  those words derive together: `columns: [container_id, image]` reads
  `CONTAINER ID   IMAGE` as two columns, the first starting at
  `CONTAINER`. Words further apart than one space are never joined.
  ...
  ID` read without `columns`). The other is a cell before the last that holds a tab or two
